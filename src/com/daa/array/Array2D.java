...
public class Array2D {
	...
	public int largestOverlap(int[][] a, int[][] b) {
		return Math.max(shiftRightAndBottom(a, b), shiftRightAndBottom(b, a));
	}

	/**
	 * @param a
	 * @param b
	 * @return max overlapping count
	 */
	private int shiftRightAndBottom(int[][] a, int[][] b) {
		int n = a.length;
		int max = 0;
		// go towards right first and then downwards		
		for (int x = 0; x < n; x++) { // shift in x direction
			for (int y = 0; y < n; y++) { // shift in y direction
				int temp = 0;
				for (int i = x; i < n; i++) { // check i from x to n as other indexes are irrelevant direction
					for (int j = y; j < n; j++) { // check j from y to n as other indexes are irrelevant direction
